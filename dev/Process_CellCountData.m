% [t_mean, t_processed] = Process_CellCountData(t_data, plate_keys, cond_keys)
%
...
function [t_mean, t_processed] = Process_CellCountData(t_data, plate_keys, cond_keys)

% assign and control the variables
if exist('plate_keys','var')
    plate_keys = unique([{'CellLine' 'Treatmentfile' 'Time'} plate_keys]);
else
    plate_keys = {'CellLine' 'Treatmentfile' 'Time'}; 
end
if exist('cond_keys','var')
    cond_keys = unique([{'DrugName' 'Conc'} cond_keys]);
else
...
EvaluateGI = any(t_data.Untrt>0 & t_data.Time==0);
...
for iP = 1:height(t_plate)
    ...
    t_ctrl = collapse(t_ctrl, @(x) trimmean(x,50), 'keyvars', {'Replicate'}, ...
        'valvars', {'Cellcount'});
    ...
    temp2 = unique(t_conditions(:, setdiff(varnames(t_conditions), ...
        {'pert_type' 'RelCellCnt' 'RelGrowth' 'Replicate' 'Ctrlcount' 'Day0Cnt' ...
        'Untrt' 'Cellcount' 'date'})));
